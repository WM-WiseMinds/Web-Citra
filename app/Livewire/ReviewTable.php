...
final class ReviewTable extends PowerGridComponent
{
    ...
    {
        return Review::query()->with(['user', 'perbaikan']);
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('user.name')
            ->add('perbaikan.bookingservice.jenis_barang')
            ->add('perbaikan.bookingservice.kerusakan')
            ->add('rating')
            ->add('comment')
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Nama Pelanggan', 'user.name')
                ->searchable()
                ->sortable(),
            Column::make('Rating', 'rating')
                ->searchable()
                ->sortable(),
            Column::make('Tanggal Review', 'created_at_formatted')
                ->searchable()
                ->sortable(),
    ...
    public function filters(): array
    {
        return [
            Filter::select('rating', 'rating')
                ->dataSource(Review::all()->unique('rating'))
                ->optionValue('rating')
                ->optionLabel('rating'),
            Filter::select('user.name', 'user_id')
                ->dataSource(Review::with('user')->get()->map(function ($review) {
                    return ['id' => $review->user_id, 'name' => $review->user->name];
                })->unique('id'))
                ->optionValue('id')
                ->optionLabel('name'),
        ];
    ...
    public function actions(Review $row): array
    {
        ...
        if (auth()->user()->can('review')) {
            $actions[] = Button::add('edit')
                ...
                ->openModal('review-form', ['rowId' => $row->id, 'perbaikan_id' => $row->perbaikan_id]);
        }
        // if (auth()->user()->can('review')) {
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.review', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/review.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/review.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $review = Review::findOrFail($rowId);
        $review->delete();
        $this->success('Review berhasil dihapus');
    }
}
