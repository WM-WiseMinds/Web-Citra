...
final class BookingServiceTable extends PowerGridComponent
{
    use WithExport;
    use Toastable;
    ...
    {
        $query = BookingService::query()->with(['user']);
    ...
    public function header(): array
    {
        ...
        if (auth()->user()->can('booking') && auth()->user()->hasRole('pelanggan')) {
            $header[] = Button::add('add-bookingservice')
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.bookingservice', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/bookingservice.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/bookingservice.pdf');
    }

    public function bulkExportPdf()
    {
        // Get the selected rows
        $selectedRows = $this->datasource()->whereIn('bookingservice.id', $this->checkboxValues)->get();
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.bookingservice', ['datasource' => $selectedRows]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/bookingservice.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/bookingservice.pdf');
    ...
    public function delete($rowId)
    {
        ...
        $this->success('Booking Service berhasil dihapus');
    }
}
